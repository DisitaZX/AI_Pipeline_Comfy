...
from chunk_prompts import split_into_chunks, get_chunk_prompts_from_ollama, substitute_entities_to_image_slots, expand_image_prompt_for_qwen_edit, precompute_all_image_prompts
...
def patch_qwen_edit_workflow(
    workflow: dict,
    image_paths: list[str],          # 1..3 absolute paths, ordered like [image 1..N]
    positive_prompt: str,
    filename_prefix: str,
    seed: int,
    negative_prompt: str | None = None,  # None → не трогаем хардкод в ноде 175
    ) -> dict:
    wf = copy.deepcopy(workflow)
    n = len(image_paths)
    if not 1 <= n <= 3:
        raise ValueError(f"image_paths должен иметь 1..3 элементов, got {n}")
...
async def main():
    ...
    """for i, scene in enumerate(plan["base_prompts"]):
        ...
        workflow_img["9"]["inputs"]["filename_prefix"] = f"base_image_gen/{scene["base_name"]}"

        workflow_img["243"]["inputs"]["value"] = 1280
        workflow_img["248"]["inputs"]["value"] = 720
    ...
    for i, scene in enumerate(plan["scenes"]):
        ...
        expanded = precomputed_image_prompts.get(sid)
        if expanded is None:
            # Сцена без entities — Gemma была пропущена. Fallback: сырой image_prompt.
            print(f"[scene {sid}] no entities, using raw image_prompt")
            expanded = scene["image_prompt"]

        image_paths = [f"{REFS_DIR}\\{name}_00001_.png" for name in entities]

        if len(image_paths) < 3 and thread_id in last_frame_per_thread:
            image_paths.append(last_frame_per_thread[thread_id])
            slot_n = len(image_paths)
            expanded = expanded + (
                f", with the character's pose and ongoing action consistent with "
                f"[image {slot_n}], rendered from the camera angle described above"
            )

        wf_qwen = patch_qwen_edit_workflow(
            workflow=workflow_qwen,
            image_paths=image_paths,
            positive_prompt=expanded,
            filename_prefix=f"image_gen/{i + 1}",
            seed=secrets.randbelow(2 ** 32),
        )

        prompt_id_img = await comfy.queue_prompt(wf_qwen)
        img_files = await comfy.wait_for_execution(prompt_id_img)
        generated_image_name = img_files[0]
        print(f"Изображение готово: {generated_image_name}")

        full_image_path = f"{GEN_IMG_DIR}\\{generated_image_name}"
        last_frame_per_thread[thread_id] = full_image_path
