...
async def main():
    ...
    p1, p2 = split_by_sentence(plan["tts_text"])
    ...
    with open('Test-Image.json', 'r', encoding='utf-8') as f:
    ...
    tts_node_id = "2"
    workflow_tts[tts_node_id]["inputs"]["target_text"] = p1
    ...
    # --- ЭТАП 1: Аудио 2 ---
    print("\n2. Запускаем генерацию TTS 2...")
    tts_node_id = "2"
    workflow_tts[tts_node_id]["inputs"]["target_text"] = p2
    ...
    for i, scene in enumerate(plan["scenes"]):
        ...
        full_prompt = "masterpiece, best quality, anime style. " + scene["image_prompt"]
        ...
        img_prompt_node_id = "8399"
        img_seed = "8398:6650"
        workflow_img[img_seed]["inputs"]["seed"] = f"{secrets.randbelow(2**32)}"
        workflow_img[img_prompt_node_id]["inputs"]["value"] = full_prompt
        workflow_img["8396"]["inputs"]["filename_prefix"] = f"image_gen/{i+1}"
    ...
    files_to_copy = ['list.txt', 'output.flac', 'audio.mp3']
    ...
    command = [
        "ffmpeg", "-i", "output.mp4", "-i", "output.flac", "-map", "0:v:0", "-map", "1:a:0", "-c:v", "copy", "-c:a", "aac", "-b:a", "320k", "output1.mp4"
    ]
